...
- **`lotus://` URL scheme** — internal pages. `lotus://history` renders `Views/Internal/LotusHistoryView.swift` as a SwiftUI overlay while web content is hidden; it is not loaded into a WKWebView. Use `URL.isLotusPage` (`Extensions/URL+Lotus.swift`) rather than raw scheme-string checks. There is no new-tab page: ⌘T opens the command palette (`Views/CommandPalette/CommandPalette.swift`, state in `BrowserState+CommandPalette`), which creates a tab only on submit — the tabs list may legitimately be empty, so never assume a selected tab exists.
...
| `Lotus/Views/Internal/`       | Internal pages (`LotusHistoryView`)                                                                                                                                                                                                       |
| `Lotus/Views/CommandPalette/` | ⌘T command palette (blurred backdrop, URL/search input, live suggestions)                                                                                                                                                                 |
